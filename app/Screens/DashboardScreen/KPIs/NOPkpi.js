...
export default function NOPkpi({
  ...
  useEffect(() => {
    if (agencyCode1) {
      const fetchAdditionalData = async () => {
        await fetchActualNOPValue();
        await fetchTargetNOPValue();
      };
      fetchAdditionalData();
    }
  ...
  const fetchActualNOPValue = async () => {
    try {
      const token = await AsyncStorage.getItem('accessToken');
  ...
  const fetchTargetNOPValue = async () => {
    try {
      const token = await AsyncStorage.getItem('accessToken');
    ...
    <View style={styles.container}>
      <TouchableOpacity onPress={handlePress} style={styles.tile}>
        <View style={styles.textContainer}>
          <Text style={styles.titleText}>NOP</Text>
    ...
    {targetValue && targetValue !== 0 ? `Target : ${targetValue}` : "Click here to set a target"}
      ...
      >
        <View style={[styles.centeredView, { padding: hp("5%") }]}>
          ...
          >
            <Text style={styles.modalText}>Set target - NOP</Text>
            <TextInput
              style={styles.input}
              onChangeText={(text) => {
